...
-- UK comps
SELECT RANK() OVER (ORDER BY num_comps DESC) AS pos, wca_id, CONCAT(name, ' (', country_id, ')'), num_comps
FROM
(
    SELECT p.wca_id, p.name, p.country_id, COUNT(DISTINCT r.competition_id) AS num_comps
    FROM persons AS p
